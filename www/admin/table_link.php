...
foreach ($grouplists as $_g) {
  if ($_g['group_id'] == $page) { $gpwd = $_g['group_pwd']; break; }
}
...
<div class="toolbar-btn-action mb-2">
  <button class="btn btn-label btn-info" id="save_order" style="display:none" onclick="save_order()">
    <label><i class="mdi mdi-checkbox-marked-circle-outline"></i></label> 保存排序</button>
  <a href="./link.php?set=add" class="btn btn-label btn-primary">
    <label><i class="mdi mdi-plus" aria-hidden="true"></i></label>新增</a>
  <button type="button" class="btn btn-label btn-success" onclick="on_link()">
    <label><i class="mdi mdi-check" aria-hidden="true"></i></label>启用</button>
  <button type="button" class="btn btn-label btn-warning" onclick="off_link()">
    <label><i class="mdi mdi-block-helper" aria-hidden="true"></i></label>禁用</button>
  <button type="button" class="btn btn-label btn-danger" onclick="del_link()">
    <label><i class="mdi mdi-window-close" aria-hidden="true"></i></label>删除</button>
  <button type="button" class="btn btn-label btn-cyan" onclick="edit_group(mv_group)">
    <label><i class="mdi mdi-account-edit" aria-hidden="true"></i></label>移动</button>
  <?php if (empty($gpwd)): ?>
    <button type="button" class="btn btn-label btn-pink" onclick="pwd_link(pwd_list)">
      <label><i class="mdi mdi-key-variant" aria-hidden="true"></i></label>加密</button>
  <?php else: ?>
    <button type="button" class="btn btn-label btn-pink" onclick="pwd_links()">
      <label><i class="mdi mdi-key-variant" aria-hidden="true"></i></label>分组已加密</button>
  <?php endif; ?>
  <a href="./batch_add.php" target="_blank" class="btn btn-label btn-purple">
    <label><i class="mdi mdi-import" aria-hidden="true"></i></label>批量导入链接</a>
  <button type="button" class="btn btn-label btn-dark" onclick="check_group_links()">
    <label><i class="mdi mdi-radar" aria-hidden="true"></i></label>检测失效</button>
</div>
<nav class="mb-3">
  <ul class="pagination">
    <?php foreach ($group_lists as $group_id => $group_name): ?>
      <li class="page-item<?php if ($page == $group_id) echo ' active'; ?>"><a class="page-link" href="?page=<?php echo $group_id; ?>"><?php echo $group_name; ?></a></li>
    <?php endforeach; ?>
  </ul>
</nav>
    ...
    <tbody id="link">
      ...
      ?>
        <tr>
          ...
          <td>
            <a class="btn btn-success btn-xs sort-goup" data-bs-toggle="tooltip" data-bs-placement="top" title="移到顶部"><i class="mdi mdi-arrow-collapse-up"></i></a>
          ...
          <td>
            <?php if ($res['link_status'] == "0"): ?><span style="color:red">禁用</span><?php else: ?><span style="color:green">启用</span><?php endif; ?>
          </td>
          <td><a href="./link.php?set=edit&id=<?php echo $res['id']; ?>" class="btn btn-xs btn-info">编辑</a> <button class="btn btn-xs btn-danger" onclick="del_link('<?php echo $res['id']; ?>')">删除</button></td>
        </tr>
      <?php
      }
      ?>
