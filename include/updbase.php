...
if ($sqlvn < $filevn) {
    ...
    if ($sqlvn < 10101) {
        $version = 'v1.1.1';
        $sql .= @file_get_contents(ROOT . 'install/data/update.sql') ?: '';
    }
    ...
    if ($sqlvn < 10103) {
        ...
        $sql .= @file_get_contents(ROOT . 'install/data/update1.sql') ?: '';
    }
    ...
    if ($sqlvn < 10105) {
        $version = 'v1.1.5';
        $sql .= @file_get_contents(ROOT . 'install/data/update2.sql') ?: '';
    }
    
    if ($sqlvn < 10106) {
        $version = 'v1.1.6';
        $sql .= @file_get_contents(ROOT . 'install/data/update3.sql') ?: '';
    }
    
    if ($sqlvn < 10109) {
        $version = 'v1.1.9';
        $sql .= @file_get_contents(ROOT . 'install/data/update4.sql') ?: '';
    }
    
    if ($sqlvn < 10200) {
        $version = 'v1.2.0';
        $sql .= @file_get_contents(ROOT . 'install/data/update5.sql') ?: '';
    }
    ...
    if ($sqlvn < 10501) {
        $version = 'v1.5.1';
        $sql .= @file_get_contents(ROOT . 'install/data/update6.sql') ?: '';
    }
    
    if ($sqlvn < 10600) {
        $version = 'v1.6.0';
        $sql .= @file_get_contents(ROOT . 'install/data/update7.sql') ?: '';
    }
    ...
    if ($sqlvn < 10800) {
        $version = 'v1.8.0';
        $sql .= @file_get_contents(ROOT . 'install/data/update8.sql') ?: '';
    ...
    if ($sqlvn < 10805) {
        ...
        if (!isset($conf['about'])) {
            ...
            if (file_exists($about_file)) {
                ...
                $about = "INSERT INTO `lylme_config` (`k`, `v`, `description`) VALUES ('about_content', '" . addslashes($about_content) . "', '关于页面');";
                $DB->query($about);
